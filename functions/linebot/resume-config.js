 ...
 * 履歷分區設定。
 *
 * 「類型」欄位只有 戲劇／廣告／學製／電影／豎屏劇 五個選項，沒有「八點檔」，
 * 所以八點檔目前靠劇名清單認。兩條路都留著：
 *   - 只要 Notion 上出現「八點檔」選項，PRIMETIME_TYPES 就會自動生效（不用改程式）
 *   - 在那之前，PRIMETIME_TITLES 是唯一的判斷依據
 *
 * 清單只對「類型 = 戲劇」的通告生效（見 resume.js 的 sectionOf）。
 * 沒列進來的戲劇一律歸「戲劇特約」—— 漏列只會分錯區，不會讓該筆從履歷消失。
 * 之後接到新的八點檔，記得回來加一行。
 */
const PRIMETIME_TITLES = [
  // 以下經 Mike 確認（2026-09-22）
  "好運來",
  "百味人生",
];
