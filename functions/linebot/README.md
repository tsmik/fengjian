...
### 1. 八點檔、豎屏劇在履歷的歸類 — 八點檔已確認，豎屏劇還沒決定

「類型」欄位只有 戲劇／廣告／學製／電影／豎屏劇，沒有「八點檔」。
現在的作法（`functions/linebot/resume-config.js`）：

- **八點檔**：靠 `PRIMETIME_TITLES` 劇名清單認，目前是 `好運來`、`百味人生`
  （2026-09-22 經 Mike 確認）。清單只對「類型 = 戲劇」生效。
  沒列進清單的戲劇會歸到「戲劇特約」，不會消失，只是分錯區 ——
  **之後接到新的八點檔要回來加一行**。
- 若之後在 Notion 的「類型」新增「八點檔」選項並回頭補標（規劃書建議的 (a) 案），
  程式不用改也會自動生效 —— `PRIMETIME_TYPES` 已經在判斷條件裡。
  長遠還是建議走 (a)：規則放在資料裡就不必每接一齣戲就改一次程式。
- **豎屏劇**：⚠️ 還沒決定，暫定跟著「戲劇特約」。要獨立一區的話在 `SECTIONS` 加一筆即可。
