...
// 13 維度代表色（淺底/深色）— 唯一正本。m_input/m_report/manual_report/manual_sens_v2/report 皆 import 此處，改色只改這裡
export const DIM_BG_COLORS=['#D6E4CC','#C8DCD8','#E2DDD5','#F0DECA','#E8D2D8','#EDE4C8','#CEDDE8','#DDD4E4','#D2DDD6','#D4E2CF','#DED5DF','#CADDD8','#CDDAE6'];
export const DIM_DEEP_COLORS=['#6B8C5A','#4A7A6E','#8A8078','#A07850','#9A6878','#9A8A50','#4A7A9A','#7A6890','#5A8A6A','#5A8A5A','#7A6088','#4A8078','#4A6E8A'];
...
// HTML 跳脫唯一正本（null/數字安全）。m_input escapeHtml、m_report _esc 皆指向此
export function _escHtml(s) { return String(s == null ? '' : s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#39;'); }
...
const ALL_PAGES = ['entry-page','mode-page','app-body','report-overlay',
  ...
  'case-page','manual-sens-page','manual-sens-v2-page','board-page','profile-page'];
