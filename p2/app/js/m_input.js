...
async function ensureDimRulesLoaded() {
  ...
  // ★ P2：規則＝上線套裝的 dims（逐側 DNF 格式，engine.js 原生可吃）
  const rs = await loadP2(db);
  if (!rs || !Array.isArray(rs.dims)) throw new Error('rules load failed (P2 套裝 dims)');
  setDimRules(rs.dims);
  _dimRulesLoaded = true;
  debugLog('[m_input]', 'P2 規則載入（套裝 ' + rs.activeRuleSetId + ' 的 dims）');
}
...
export async function ensureQuestionsLoaded() {
  ...
  _questionsPromise = (async () => {
    // ★ P2：題目＝上線套裝的 observations（轉成 OBS_PARTS_DATA 形狀，ruleset_convert 已對齊）
    try {
      const rs = await loadP2(db);
      if (rs && rs.obsParts && Object.keys(rs.obsParts).length > 0) {
        setObsPartsData(rs.obsParts);
        setObsPartNames(rs.partNames);
        _questionsLoaded = true;
        debugLog('[m_input]', 'P2 題目載入（套裝 ' + rs.activeRuleSetId + ' 的 observations）');
        return;
      }
      debugLog('[m_input]', 'P2 套裝 observations 空，改用內建預設');
    } catch (e) {
      debugLog('[m_input]', 'P2 套裝題目讀取失敗', e && e.message);
    }
...
// 捲動容器（桌機維度視角：第2欄部位導覽、第3欄條件欄 各自獨立捲動；整頁＝.m-main）
const _SCROLL_SELS = ['.m-main', '.m-dimv-partnav', '.m-dimv-condcol'];
function _captureScroll(sels) { const m = {}; sels.forEach(s => { const el = document.querySelector(s); if (el) m[s] = el.scrollTop; }); return m; }
function _restoreScroll(m) { Object.keys(m).forEach(s => { const el = document.querySelector(s); if (el) el.scrollTop = m[s]; }); }
// 答題後重繪：三欄全保留（停在原地）
function _renderKeepScroll() { const m = _captureScroll(_SCROLL_SELS); render(); _restoreScroll(m); }
// 選部位/維度後重繪：保留第2欄(部位導覽)+整頁；第3欄(條件欄)因重建自然從頂顯示新部位條件
function _renderNav() { const m = _captureScroll(['.m-main', '.m-dimv-partnav']); render(); _restoreScroll(m); }
...
function _persistSpice(lv) {
  if (!window.__userData) window.__userData = {};
  window.__userData.spice = lv;
  const uid = getEffectiveUid();   // per-學員：一律存自己 users/{uid}，不存個案
  if (!uid) return;
  setDoc(doc(db, 'users', uid), { spice: lv, updatedAt: new Date().toISOString() }, { merge: true }).catch(() => {});
}

function renderObsReport() {
  _applySavedSpiceOnce();                   // 還原學員存的辣度（一次，先於算矩陣）
  const seg = renderSegmented();
  ...
  if (ud.gender && ud.birthday) {
    if (isLiunianReady()) {
      ...
      ensureLiunianLoaded().then(() => { if (_view === 'report' && isLiunianReady()) render(); }).catch(() => {});
    }
  }
...
const SPOV_RADAR_CFG = {   // 係數圖:部分維度名直書(vert)
  scale: 1.12, offX: 0, offY: 8, fs: { dn: 13.5, dv: 15.5 }, hideRole: {},
  vert: { "dn-0":false,"dn-2":true,"dn-3":true,"dn-9":true,"dn-10":true },
  moved: {
    "dn-0":{x:243.3,y:54.8},"dn-1":{x:308.5,y:92.4},"dn-2":{x:366.2,y:142.4},"dn-3":{x:373.1,y:234.4},"dn-4":{x:335.1,y:332.1},"dn-5":{x:272.9,y:385.3},"dn-6":{x:201.5,y:401.4},"dn-7":{x:125.4,y:382},"dn-8":{x:64,y:326.9},"dn-9":{x:24.9,y:261.1},"dn-10":{x:32.7,y:172.2},"dn-11":{x:91.1,y:94.9},"dn-12":{x:161.7,y:57.3},
    "dv-0":{x:236.8,y:84.7},"dv-1":{x:298,y:119.3},"dv-2":{x:331.7,y:172.5},"dv-3":{x:340.6,y:249.3},"dv-4":{x:315.8,y:312.5},"dv-5":{x:270.3,y:358.3},"dv-6":{x:199.8,y:380.5},"dv-7":{x:131.5,y:355.5},"dv-8":{x:86.2,y:316.1},"dv-9":{x:59.1,y:246.1},"dv-10":{x:66.8,y:178.9},"dv-11":{x:103.1,y:117.5},"dv-12":{x:165.8,y:82.2}
  }
};
const SPOV_R3_CFG = {   // 動靜圖:外圍係數(dv)刪除,維度名(dn)+圖內動靜數字(pole)自訂;部分維度名直書(vert)
  scale: 1.11, offX: 0, offY: 2, fs: { dn: 14, dv: 15.5, pole: 14 }, hideRole: { dv: true },
  vert: { "dn-1":false,"dn-2":true,"dn-3":true,"dn-4":false,"dn-8":false,"dn-9":true,"dn-10":true },
  moved: {
    "dn-0":{x:244,y:52},"dn-1":{x:307.9,y:86},"dn-2":{x:363.2,y:135.4},"dn-3":{x:370.4,y:228.2},"dn-4":{x:334.1,y:325.7},"dn-5":{x:272.7,y:379.9},"dn-6":{x:200.7,y:395.9},"dn-7":{x:130,y:376.3},"dn-8":{x:62.2,y:326},"dn-9":{x:30.1,y:260.7},"dn-10":{x:35.9,y:167.8},"dn-11":{x:88.7,y:95.7},"dn-12":{x:156.2,y:51.7},
...
const SPOV_COEF_HS = 1;   // 係數總覽高度倍率(在 spov_tuner2 調定後填入;>1 = 更高、字更大)
...
function _buildSpovGridHtml() {
  ...
  const cols = LEVELS.map(lv => {
    ...
    const p = buildManualReportParts(matrix, { noChartTitle: true, hideRadarCenter: true, coefColorMode: 'byType', coefHeightScale: SPOV_COEF_HS });
...
function _collectRefsFromNode(node, out) {
  if (!node) return;
  if (Array.isArray(node)) { node.forEach(n => _collectRefsFromNode(n, out)); return; }
  if (node.ref !== undefined) { out.add(node.ref); return; }
  if (node.partResult !== undefined) return;
  // P2 格式：part → cards → combos → leaves
  if (node.cards) node.cards.forEach(c => _collectRefsFromNode(c, out));
  if (node.combos) node.combos.forEach(cb => _collectRefsFromNode(cb, out));
  if (node.leaves) node.leaves.forEach(l => _collectRefsFromNode(l, out));
  // P1 格式：items/item/each/rule
...
function bindEvents() {
  ...
  // 辣度選擇器（F3）：切換 → 立即高亮(即時回饋) + debounce 重算重繪 + debounce 寫入
  // （重建整份報告含 4 張 SVG 圖較重；快速連點只在停下後重繪/寫入一次，避免卡頓）
  _root.querySelectorAll('[data-spice]').forEach(btn => {
    btn.addEventListener('click', () => {
      const lv = btn.dataset.spice;
      if (lv === getSpice()) return;
      setSpice(lv);
      // 立即把高亮切到所選辣度（便宜的 DOM 更新，點擊馬上有反應）
      _root.querySelectorAll('[data-spice]').forEach(b => b.classList.toggle('is-on', b.dataset.spice === lv));
      clearTimeout(_spiceRenderTimer);
  ...
  _root.querySelectorAll('.m-sv-dim').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      _renderNav();   // 選維度：保留部位導覽+整頁捲動，不跳回頂
    });
  });
  ...
  _root.querySelectorAll('.m-dimv-part[data-pi]').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      _renderNav();   // 選部位：保留第2欄部位導覽+整頁捲動（不再 _scrollDimvTop 跳頂）；第3欄重建自然從頂顯示新條件
    });
  });
